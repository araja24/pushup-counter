...
def test_frames_before_start_report_idle_and_never_a_rep(socket):
    frames = synthetic.generate_frames(cycles=2)

    for frame in frames:
        socket.send_json(frame)
    socket.send_json({"cmd": "start", "set_id": "set-1"})
    messages = read_until(socket, "state")

    assert kinds(messages, "rep") == []
    assert all(state["phase"] == "IDLE" for state in kinds(messages, "state")[:-1])
    assert all(state["reps"] == 0 for state in kinds(messages, "state"))
